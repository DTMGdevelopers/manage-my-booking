...
if [ -f "$cache_file" ] && [ $(( $(date +%s) - $(stat -c %Y "$cache_file") )) -lt $cache_ttl ]; then
  ...
  source "$cache_file"
    ...
    echo "ca_tt_sitename=\"$(mysql --login-path=local -N --execute="USE $ca_db_name; SELECT option_value FROM ${ca_db_table_prefix}_options WHERE option_name = 'options_traveltek_domain';")\""
